...
def main(pretrained="", augment=False, num_augs=num_augs):
    for max_points in tqdm(max_points_list, desc="Total: ", leave=False, colour="blue"):
        ...
        # Model Name
        model_name = f"DGCNN_{max_points}_{len(classes)}"
        ...
        # Define model
        model = PointTransformer(num_features=len(use_columns), num_classes=len(classes), dim_model=[32, 64, 128, 256, 512], k=16)
