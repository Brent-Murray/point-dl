...
def objective(trial):
    hp = {
        ...
        "batch_size": trial.suggest_int("batch_size", low=4, high=32, step=2),  # batch size *
        ...
        "step_size": trial.suggest_int("step_size", low=5, high=50, step=5),  # step size *
    }
...
if __name__ == "__main__":
    ...
    textio.cprint("Study Statistics: ")
    textio.cprint(f"   Number of finished tirals: {len(study.trials)}")
    textio.cprint(f"   Number of pruned trials: {len(pruned_trials)}")
    textio.cprint(f"   Number of complete trials: {len(complete_trials)}")
    textio.cprint("Best Trial:")
    trial = study.best_trial
    textio.cprint(f"  Value: {trial.value}")
    textio.cprint("  Params: ")
    ...
    plt.savefig(f"outputs/{model_name}_importance.png")